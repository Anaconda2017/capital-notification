...
@php $pageTitle = ' NewsLetter Control'; @endphp
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
               ...
               <a href="{{ route('newsletter.create') }}"><button class="btn light btn-primary ">Add</button></a> 
              </div>
              <div class="card-body">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-bold-500">
                                    {{ $coupon->newsletter_title }}
                                </td>
                                <td class="text-bold-500">
                                    {{ $coupon->newsletter_date }} - {{ $coupon->newsletter_time }}
                                </td>
                                
                                @if($coupon->newsletter_status == 1)
                                ...
                                <td>
                                    <div class="d-flex">
                                        <a href="{{ route('newsletter.edit', $coupon->id) }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
                                        
                                        
                                        @if($coupon->newsletter_status == 1)
                                            <form action="{{ route('newsletter.destroy', $coupon->id)  }}" method="POST">
                                                {{ method_field('delete') }}
                                                {{ csrf_field() }} 
                                        ...
                                        @else
                                                <form action="{{ route('newsletterRecover', $coupon->id)  }}" method="POST">
                                                    @csrf 
